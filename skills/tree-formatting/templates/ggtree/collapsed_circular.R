...
# Optional: load accession -> gene name mapping
# See gene-lookup skill for how to generate this from UniProt, Ensembl, etc.
...
BRANCH_LINE_WIDTH  <- 0.15    # branch thickness (also triangle outlines)
TRIANGLE_ALPHA     <- 0.4     # triangle fill transparency
TRIANGLE_MODE      <- "max"   # "max" = rectangular triangles

# Label styling (circular)
LABEL_SIZE_CIRC    <- 1.0     # geom_text size for labeled circular
LABEL_OFFSET_CIRC  <- 0.005   # offset from tip to label start (circular coords)
...
# Page dimensions
PAGE_SIZE_OVERVIEW  <- 20     # inches — square page for unlabeled overview
PAGE_SIZE_LABELED   <- 28     # inches — square page for labeled version
...
# Branch length capping
BRANCH_CAP_PERCENTILE <- 0.90
...
# BUILD CIRCULAR PLOT
# =============================================================================
# CRITICAL GOTCHA: Label positions must be pre-computed BEFORE collapse().
# collapse() modifies p$data coordinates, so geom_tiplab2() reads wrong positions.
# Instead, we compute angles manually from the y-position in circular coordinates
# and use geom_text() with explicit x, y, and angle values.

build_circular_plot <- function(tree, tip_data, node_group, clades_to_collapse,
                                line_width = BRANCH_LINE_WIDTH,
                                show_labels = FALSE,
                                label_size = LABEL_SIZE_CIRC,
                                label_offset = LABEL_OFFSET_CIRC,
                                do_collapse = TRUE) {
  ...
  if (show_labels) {
    ...
    if (nrow(visible_rows) > 0) {
      ...
      # Match on node column — do NOT assume row ordering
      data_row_idx <- match(tip_node_ids, pre_data$node)

      # Compute angle from y-position for circular layout
      max_y <- max(pre_data$y, na.rm = TRUE)
      angles <- pre_data$y[data_row_idx] / max_y * 360
      ...
      # Offset direction: positive for right-reading, negative for flipped
      offset_sign <- ifelse(flip, -1, 1)

      tip_label_df <- data.frame(
        x = pre_data$x[data_row_idx] + label_offset * offset_sign,
  ...
  # angle and hjust are passed outside aes() because they are per-row constants,
  # not mapped aesthetics. show.legend = FALSE prevents "a" artifacts in legend.
...
# --- Circular overview (no labels) ---
cat("\nBuilding circular overview...\n")
p_overview <- build_circular_plot(tree, tip_data, node_group, clades_to_collapse,
                                  show_labels = FALSE)
out_overview <- file.path(out_dir, "GENE_ggtree_circular.pdf")
ggsave(out_overview, p_overview, width = PAGE_SIZE_OVERVIEW,
       height = PAGE_SIZE_OVERVIEW, units = "in")
cat("Saved:", out_overview, "\n")

# --- Circular with selective labels ---
cat("Building circular with labels...\n")
p_labeled <- build_circular_plot(tree, tip_data, node_group, clades_to_collapse,
                                 show_labels = TRUE,
                                 label_size = LABEL_SIZE_CIRC,
                                 label_offset = LABEL_OFFSET_CIRC)
out_labeled <- file.path(out_dir, "GENE_ggtree_circular_labeled.pdf")
ggsave(out_labeled, p_labeled, width = PAGE_SIZE_LABELED,
       height = PAGE_SIZE_LABELED, units = "in")
cat("Saved:", out_labeled, "\n")
